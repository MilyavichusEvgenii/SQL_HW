...
INSERT sales(order_rate, count_product)
VALUES
	('2022-01-01', 156),
    ('2022-01-02', 180),
	('2022-01-03', 21),
    ...
    SELECT
        ...
        if(count_product >= 100 && count_product <= 300, "Средний заказ",
        if(count_product > 300, "Большой заказ", "Заказ не оформлен")))
        AS "Тип заказа"
	FROM sales;
...
SELECT posts.*, (SELECT login FROM users
WHERE posts.user_id=users.id)login FROM posts;

-- №3.	Выполните поиск  по публикациям, автором котоырх является клиент "Mikle".
SELECT posts.*, users.login FROM posts, users
WHERE posts.user_id = users.id AND users.login = 'Mikle';
